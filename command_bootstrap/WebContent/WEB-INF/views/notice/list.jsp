...
<body>
			...
			<div class="card-body">
				<table class="table table-bordered text-center" >					
					...
					<c:forEach items="${noticeList }" var="notice" >
						<tr style='font-size:0.85em;cusor:pointer;' onclick="OpenWindow('detail.do?nno=${notice.nno}','상세보기','800','700');">
						  <td>${notice.nno }</td>
    ...
    window.onload=function(){
    	$("[data-target='notice-writer']").click(function(event){
    		OpenWindow('<%=request.getContextPath()%>/member/detail.do?id='+$(this).text(),'상세보기',800,700);
    		return false;
    	});
    }
    </script>
